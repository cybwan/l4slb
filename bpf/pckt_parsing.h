...
struct quic_long_header {
  ...
  // For routing, flomesh lb requires minimum of 'QUIC_MIN_CONNID_LEN',
...
parse_quic(void *data, void *data_end, bool is_ipv6,
           struct packet_description *pckt) {
  struct quic_parse_result result = {.server_id = FURTHER_PROCESSING,
                                     .is_initial = false};
  ...
  // even if the connid len can be of 0 or 4-18 bytes, since flomesh lb is only
